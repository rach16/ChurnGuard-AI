"""
RAG Data Ingestion Script (Server Mode)
Loads synthetic data into Qdrant server running on localhost:6333
"""
...
class RAGDataIngestion:
    ...
    def __init__(self):
        ...
        logger.info(f"Using Qdrant server at {self.qdrant_url}")
...
{chr(10).join(interaction_list[:10])}  # Show up to 10 most recent
    ...
    def create_vectorstore(self, documents: List[Document]):
        ...
        # Create vectorstore - connect to running server
    ...
    def ingest_all(self):
        ...
        logger.info("=" * 60)
        logger.info("Starting RAG data ingestion pipeline (Server Mode)")
        logger.info("=" * 60)
        ...
        logger.info("=" * 60)
        logger.info(f"\n🌐 Vector database server: {QDRANT_URL}")
        ...
        logger.info(f"  3. Verify in Qdrant dashboard: {QDRANT_URL}/dashboard")

        return vectorstore

def main():
    """Run ingestion pipeline"""
    ingestion = RAGDataIngestion()
